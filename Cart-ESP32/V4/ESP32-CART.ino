...
int minSpeedLevel = 3; // Default minimum speed level (1-10)
  ...
  if (SerialBT.available()) {
    ...
    switch(command) {
      ...
      case '5': case '6': case '7': case '8': case '9': {
        // Wrapped in scope to avoid cross-initialization errors
        int speedLevel = minSpeedLevel + (command - '0');
        // Clamp to max level 10 (100% speed)
        if (speedLevel > 10) speedLevel = 10;
        // Convert to PWM (level 10 = 255, level 1 = 25.5, etc.)
        currentSpeed = speedLevel * 25.5;
        lastAction = "Speed Level " + String(speedLevel) + " (" + String(currentSpeed) + " PWM)";
        break;
      }
      case 'q':
        currentSpeed = 255;
...
void loop() {
  ...
  if (powerOn && operationMode == MODE_AUTONOMOUS && !overloaded) {
    ...
    else if (distanceLeft <= 40 || distanceCenter <= 40 || distanceRight <= 40) {
      stopMotors();
