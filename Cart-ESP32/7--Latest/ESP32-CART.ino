...
// Send ultrasonic data to Python via Bluetooth
void sendUltrasonicData() {
  long left = getSequentialDistance(TRIG_LEFT, ECHO_LEFT);
  delay(10);
  long center = getSequentialDistance(TRIG_CENTER, ECHO_CENTER);
  delay(10);
  long right = getSequentialDistance(TRIG_RIGHT, ECHO_RIGHT);
  
  String data = "US:" + String(left) + "," + String(center) + "," + String(right);
...
void loop() {
  ...
  // Pick-up detection for all modes
  if (powerOn) {
  ...
  if (powerOn && (operationMode == MODE_MANUAL || operationMode == MODE_OPENCV || operationMode == MODE_HYBRID)) {
    handleBluetoothCommands();
    
    // Special handling for hybrid mode - send ultrasonic data to Python
    if (operationMode == MODE_HYBRID && !cartPickedUp) {
      unsigned long currentMillis = millis();
      if (currentMillis - lastUltrasonicSendTime >= 100) { // Send every 100ms
        sendUltrasonicData();
        lastUltrasonicSendTime = currentMillis;
      }
    }
  }
}