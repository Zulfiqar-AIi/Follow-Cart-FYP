...
// Web Interface Handler (unchanged)
void handleRoot() {
  ...
  // Bluetooth info (if in manual/OpenCV mode)
  if(operationMode == MODE_MANUAL || operationMode == MODE_OPENCV) {
    page += "<div class='status-box'>";
...
void handleBluetoothCommands() {
  ...
  if (SerialBT.available()) {
    ...
    switch(command) {
      case 'F': moveForward(currentSpeed, currentSpeed); lastAction = "Forward"; break;
      case 'B': moveBackward(currentSpeed, currentSpeed); lastAction = "Backward"; break;
      case 'L': turnLeft(); lastAction = "Turning Left"; break;
      case 'R': turnRight(); lastAction = "Turning Right"; break;
      case 'S': stopMotors(); lastAction = "Stopped"; break;
      case 'D': emergencyStop(); lastAction = "Emergency Stop"; break;
...
void loop() {
  ...
  // Read weight every 500ms - SKIP IN MANUAL/OPENCV MODES TO AVOID BLUETOOTH DELAYS
  ...
  // Stop cart & activate buzzer if overloaded (unchanged)
  ...
  // Pick-up detection (Autonomous mode only) (unchanged)
  ...
  // Handle Bluetooth in manual/OpenCV modes - FIXED TO AVOID DELAYS
  if ((operationMode == MODE_MANUAL || operationMode == MODE_OPENCV) && powerOn) {
  ...
  if (powerOn && operationMode == MODE_AUTONOMOUS && !overloaded) {
    // IR signal check (unchanged)
    if (IrReceiver.decode()) {
      if (IrReceiver.decodedIRData.protocol == 2) {
